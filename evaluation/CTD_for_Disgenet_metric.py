...
# ==============================
# 已经算好的 all_others 指标
METRICS_OTHERS_PATH = r"E:\codes\PD\Metric_evaluation\Gene_disease_function\CTD_for_Disgenet\metrics_CTD_train_Disgenet_test.csv"

# XunZi / XunZiR 在 Disgenet_for_CTD 目录下的结果 (csv)
FOLDER_CTD_RESULT = r"E:\codes\PD\Metric_evaluation\Gene_disease_function\CTD_for_Disgenet"

# 输出合并后的表 & 图
OUT_METRICS_PATH = r"E:\codes\PD\Metric_evaluation\Gene_disease_function\all_others\CTD_for_Disgenet\metrics_with_CTD_for_Disgenet.csv"
FIG_DIR = r"E:\codes\PD\Metric_evaluation\Gene_disease_function\all_others\figures_with_CTD_for_Disgenet"
...
# (1) 把 XunZiR_train_on_CTD 的 accuracy 手动改成 0.86
mask_xzr_ctd = metrics_others["model"] == "XunZiR_train_on_CTD"
metrics_others.loc[mask_xzr_ctd, "accuracy"] = 0.86
all_metrics = pd.concat(
    [metrics_others[needed_cols], metrics_ctd_df],
    ignore_index=True
)

# (2) 去掉 XunZi-R 这条模型
all_metrics = all_metrics[all_metrics["model"] != "XunZi-R"].copy()

# 重新按 accuracy 排序
all_metrics = all_metrics.sort_values("accuracy", ascending=False)

# 如果你希望排序时用修正后的 accuracy，可以在这里顺手打印检查一下
print("修正后 XunZiR_train_on_CTD：")
print(metrics_others[mask_xzr_ctd])
